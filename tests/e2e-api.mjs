...
try {
  ...
  console.log(JSON.stringify({ status: 'PASS', marketId: market.id, policyLocked: true, evidenceHash: resolved.body.data.evidenceHash, unauthorizedResolutionRejected: true, evidenceMismatchRejected: true, earlyClaimRejected: true, latePositionRejected: true, doubleClaimRejected: true }))
} finally {
  child.kill('SIGTERM')
}
